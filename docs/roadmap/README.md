...
The current interface inherits a **Caelestia-style** layout and information architecture—dense tiles, control-center concepts, and a cohesive “shell OS” feel. That baseline is intentional and appreciated. At the same time, the UI will **evolve**: layout, grouping, colors, and which surfaces live in GTK (`src/widget/`) versus React (`ui/`) versus the sidecar will shift as features land. Roadmap docs call out **configurable and modular** panels where the todo asks for drag-and-drop, quickviews, or per-device controls.
...
| [sidebar.md](./sidebar.md) | Caelestia-style sidebar: layout, tiles, workspaces, task manager, connectivity tiles |
...
| [pentest-panel.md](./pentest-panel.md) | Pentest workflow surface (stub) |
