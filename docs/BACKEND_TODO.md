...
Logs `FollowLogs` → `Logs.Line` WS; weather cache + `AURA_WEATHER_API_KEY`; `Performance.ApplyPreset`; DevOps timer fixtures; automation localhost webhook; CalDAV read-only `Calendar.SyncCalDav`; `Launcher.VicinaeQuery`; Vault `GetEntry`/`SetEntry`; greenfield ADRs (Communication, IDE, voice, gamemode doc).

### Partial / deferred

Network `Connect` (802.1X, captive portal), VPN per-app routing, CalDAV two-way/recurrence, `Vault.Export`/`Transfer`, Communication hub depth, focus mode / site blocking, AUR/Flatpak depth, GTK `sidecar.ts` push parity for all namespaces.
...
| Item | Status |
|------|--------|
| Registered RPC methods (default build) | See [`sidecar/rpc-manifest.json`](../sidecar/rpc-manifest.json) |
| Services registered | Same as prior + `caldav` helpers in `calendar`/`caldav.rs` |
| Integration tests | **310+** test fns — includes `caldav_rpc_shapes`, `vault_storage_test`, extended `server_http_test` |
...
| Storage | SQLite KV — list/delete/scan, migrations; `vault_audit` namespaces |
...
| **P4** | Stubs / optional | Exocortex, voice |
...
- [~] Port scan safety caps (defensive `Security.ScanPorts`)
...
| 7 | [greenfield_stubs_platform.md](plans/completion/greenfield_stubs_platform.md) — **done** | §3.9–3.14, §4 ADRs |
...
| 2026-05-31 | Batch 4 — calendar/todos ICS, vault RO, security gate | plan commits on `master` |
