...
**Repo:** `~/.config/ags` (or any clone with `AURA_SIDECAR` set — see [Binary path](#binary-path-caveat) below)
...
- [ ] **Binary path caveat:** document in README + [BACKEND_TODO.md](../BACKEND_TODO.md) §0.1 — `src/lib/sidecar.ts` looks under `$XDG_CONFIG_HOME/ags/sidecar/target/...`; other clones must set `AURA_SIDECAR` or build into that path
...
5. **Repo canonical path:** `~/.config/ags` for AGS load; other clones set `AURA_SIDECAR`.
