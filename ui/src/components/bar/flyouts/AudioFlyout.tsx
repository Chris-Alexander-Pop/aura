...
export default function AudioFlyout() {
  ...
  const busy =
    sinkVolMut.isPending || sinkMuteMut.isPending || sourceMuteMut.isPending || setDefaultMut.isPending
  ...
  return (
    <div className="flex flex-col gap-3 px-3 pb-3 pt-3 text-text">
      ...
      {defaultSource ? (
        <div className="flex items-center justify-between gap-2 border-t border-surface0/50 pt-2">
          <div className="min-w-0">
            <p className="text-[10px] uppercase tracking-wide text-subtext1">Mic</p>
            <p className="truncate text-xs text-subtext0">{defaultSource.name}</p>
          </div>
          <button
            type="button"
            disabled={busy}
