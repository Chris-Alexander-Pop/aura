...
export function PerformancePane() {
  ...
  const killMut = useMutation({
    mutationFn: (row: { pid: number; confirmation_token: string }) =>
      api.processKill(row.pid, row.confirmation_token),
            ...
            {processesQuery.data!.map((p) => (
              <li key={p.pid} className="flex items-center justify-between gap-2">
                ...
                <button
                  ...
                  onClick={() =>
                    killMut.mutate({ pid: p.pid, confirmation_token: p.confirmation_token })
                  }
                >
