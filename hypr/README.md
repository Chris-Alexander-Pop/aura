...
hypr/
  ...
  hyprland/
    ...
    user.lua                    # machine overrides (devices)
...
Host-only autostart (Waydroid, HDMI reinit, etc.): copy `hypr/hyprland/execs-local.lua.example` to `execs-local.lua` (gitignored). Same pattern for `monitors-local.lua`.
