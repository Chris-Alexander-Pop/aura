...
const DENIED_PREFIXES: &[&str] = &["Session."];
...
impl ExecFixtureGuard {
    ...
    pub fn activate_dir(dir: PathBuf) -> Self {
        ...
        // Live gitignored ~/.config/ags/vpn/profiles.json must not leak into tests.
        let previous_vpn = std::env::var("AURA_VPN_CONFIG_DIR").ok();
        std::env::set_var("AURA_VPN_CONFIG_DIR", dir.to_string_lossy().as_ref());
        Self {
            previous,
            previous_vpn,
        }
...
impl Drop for ExecFixtureGuard {
    fn drop(&mut self) {
        ...
        if let Some(prev) = self.previous_vpn.take() {
            std::env::set_var("AURA_VPN_CONFIG_DIR", prev);
        } else {
...
mod guard_tests {
    ...
    #[test]
    fn denies_session_prefix_and_exact_mutators() {
