...
fn sanitize_vpn_profile_id(profile_id: &str) -> String {
    let s: String = profile_id
        .chars()
...
/// Auth file for `--auth-user-pass` — never world-readable `/tmp`.
pub(crate) fn vpn_auth_file_path(profile_id: &str) -> PathBuf {
    let runtime = std::env::var_os("XDG_RUNTIME_DIR")
        .map(PathBuf::from)
        .filter(|p| p.is_absolute())
        .unwrap_or_else(|| {
            std::env::temp_dir().join(format!("aura-{}", std::process::id()))
        });
    runtime
        .join("aura")
        .join(format!(
            "vpn-auth-{}.txt",
            sanitize_vpn_profile_id(profile_id)
        ))
}

async fn write_vpn_auth_file(profile_id: &str, username: &str, password: &str) -> Result<PathBuf> {
    let path = vpn_auth_file_path(profile_id);
...
async fn connect_openvpn(
    ...
    let auth_file = write_vpn_auth_file(profile_id, &username, &password).await?;
    let auth_path = auth_file.to_string_lossy().into_owned();
...
mod tests {
    use super::{
        ...
        parse_wireguard_conf, snapshot_vpn_status, vpn_auth_file_path, vpn_interface_connected,
        VpnProcess,
    ...
    #[test]
    fn vpn_auth_file_is_not_world_tmp() {
        let home = vpn_auth_file_path("home");
        assert!(
            home.ends_with("aura/vpn-auth-home.txt"),
            "{}",
            home.display()
        );
        assert!(!home
            .to_string_lossy()
            .contains("/tmp/aura-vpn-auth-"));
        let sneaky = vpn_auth_file_path("../etc/passwd");
        assert_eq!(
            sneaky.file_name().and_then(|s| s.to_str()),
            Some("vpn-auth-etcpasswd.txt")
        );
    }
