...
pub fn register(registry: &mut ServiceRegistry) {
    ...
    registry.register("Keybinds.Import", |params| async move {
        ...
        for e in &entries {
            assert_bind_action_writable(&e.action)?;
        }
    ...
    registry.register("Keybinds.Set", |params| async move {
        ...
        assert_bind_action_writable(&action)?;
...
fn action_mentions_exec(action: &str) -> bool {
    action
        .to_ascii_lowercase()
        .split(|c: char| !c.is_ascii_alphanumeric() && c != '_')
        .any(|t| matches!(t, "exec" | "execr" | "execdispatcher" | "exec_cmd"))
}

fn assert_bind_action_writable(action: &str) -> Result<()> {
    let a = action.trim();
    if action_mentions_exec(a) {
        anyhow::bail!("keybind writes cannot include exec");
    }
    let cmd = extract_dispatch(a).unwrap_or_else(|| a.to_string());
    crate::services::hyprland::validate_dispatch(&cmd)
        .map(|_| ())
        .map_err(|e| anyhow::anyhow!("keybind write denied: {e}"))
}
...
"#;
    ...
    #[test]
    fn writable_action_rejects_exec() {
        assert!(assert_bind_action_writable("exec kitty").is_err());
        assert!(assert_bind_action_writable("dispatch exec foot").is_err());
        assert!(assert_bind_action_writable("killactive").is_ok());
        assert!(assert_bind_action_writable("dispatch workspace 2").is_ok());
    }
