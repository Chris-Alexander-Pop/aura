...
"#,
    ...
    #[tokio::test]
    async fn lookup_google_oauth_refresh_missing_binary_returns_none() {
        let _guard = keyring_test_lock().lock().await;
        let prev = std::env::var("PATH").ok();
        // Empty PATH so `secret-tool` cannot be found.
    ...
    #[tokio::test]
    async fn clear_google_oauth_refresh_missing_binary_is_ok() {
        let _guard = keyring_test_lock().lock().await;
        let prev = std::env::var("PATH").ok();
        std::env::set_var("PATH", "");
        clear_google_oauth_refresh().await.expect("clear");
        match prev {
            Some(p) => std::env::set_var("PATH", p),
            None => std::env::remove_var("PATH"),
        }
